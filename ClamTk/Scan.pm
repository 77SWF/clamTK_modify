...
sub filter_new {
    ...
    $hb = Gtk3::HeaderBar->new;
    $window->set_titlebar( $hb );
    $hb->set_title( _( '[-第三组-]新算法加载中...' ) );
    $hb->set_show_close_button( TRUE );
    $hb->set_decoration_layout( 'menu:close' );

    $window->signal_connect(
        'destroy' => sub {
            if ( !$stopped_new ) {#如果进程在运行
                return TRUE;
            } else {#停止运行了 可以点其他界面了
                $window->destroy;
                1;
    ...
    my $eb = Gtk3::EventBox->new;
    $window->get_content_area->add( $eb );#主要区域放EventBox

    my $box = Gtk3::VBox->new( FALSE, 5 );
    $eb->add( $box );#垂直盒加进EventBox
    
    my $hbox = Gtk3::HBox->new( FALSE, 0 );
    $box->add( $hbox );#水平盒加进垂直盒

    $topbar = Gtk3::InfoBar->new;
    $hbox->pack_start( $topbar, TRUE, TRUE, 5 );#水平盒加InfoBar，进度文字

    Gtk3::main_iteration while Gtk3::events_pending;
    $topbar->set_message_type( 'other' );
    set_infobar_text( $topbar, _( '正在扫描...' ) );#准备中
    Gtk3::main_iteration while Gtk3::events_pending;

    reset_stats();#！！！去加个$stopped_new=1
#resules!!!
    #检查结果还没加
    $scan_results
        = Gtk3::Label->new;
    $scan_results->set_line_wrap(TRUE);
    $scan_results->set_ellipsize (none);
    my $text_box = Gtk3::VBox->new( FALSE, 5 );
    $text_box->add( $scan_results );
    $box->pack_start( $text_box, FALSE, FALSE, 5 );
    ...
    $box->pack_start( $bottombar, FALSE, FALSE, 5 );
    $bottombar->set_message_type( 'other' );
    $use_image = 'gtk-cancel';#扫描的时候取消按钮
    $bottombar->add_button( $use_image, HATE_GNOME_SHELL );
    if ( $show ) {
        $use_image = ClamTk::Icons->get_image( 'preferences-system' );
        $bottombar->add_button( $use_image, DESTROY_GNOME_SHELL );
...
sub scan_new {
    ...
    #my $result=readpipe("python3.8 /usr/share/perl5/ClamTk/test.py $quoted");
    #print($result);
    #system("python3.8 /home/t/桌面/test/test.py $quoted");
    $scan_pid = open( $SCAN, '-|', "python3 /home/t/桌面/test/test.py /home/t/桌面/exefiles" );
    #命令行的命令
	...
	#resules!!!
	$new_scan_results .= $_.'|';
	$scan_results->set_text($new_scan_results);
...
sub clean_up_new {
    ...
    my $message = '';
    if ( !$found_count_new ) {
        $message = _( '自定义扫描算法扫描完成，结果如下：' );
    } else {
        $message = _( '发现可能的恶意程序，请进一步使用原生scanner扫描判断类型' );
    }
    set_infobar_text( $topbar, $message );

}

sub select_directory_new {
    my $directory = '';
