...
sub start_gui {
    ...
    # new
    $theme->append_search_path( '/usr/share/icons/gnome/24x24/new' );
...
sub add_configuration {
    ...
    my $label = Gtk3::Label->new;
    $label->set_markup( "<u><b>$show_this</b></u>" );
...
sub iconview_react {
    ...
    if ( $value eq _( 'About' ) ) {
        ...
        return TRUE;
    } elsif ( $value eq _( '(新)扫描文件' ) ) {
        select_file_new();
        return TRUE;
    } elsif ( $value eq _( '（新）扫描目录' ) ) {
        select_directory_new();
        return TRUE;
    }
...
sub  select_file_new {
    my $file   = '';
    my $dialog = Gtk3::FileChooserDialog->new(
        _( 'Select a file' ), $window,#左上角选择文件，父界面是主界面
        'open',#打开（另一个是save）
        'gtk-cancel' => 'cancel',#图标=>文字（按了后接收的值）
        'gtk-ok'     => 'ok',
    );
    $dialog->set_select_multiple( FALSE );#不可多选
    if ( ClamTk::Prefs->get_preference( 'ScanHidden' ) ) {
        $dialog->set_show_hidden( TRUE );
    }
    $dialog->set_position( 'center-on-parent' );
    if ( 'ok' eq $dialog->run ) {#如果点击OK
        $window->queue_draw;
        Gtk3::main_iteration while Gtk3::events_pending;
        $file = $dialog->get_filename;#文件名
        $dialog->destroy;
        $window->queue_draw;
    } else {#点击取消
        $dialog->destroy;
    ...
    if ( -e $file ) {#-e 文件或目录名存在
        ClamTk::Scan->filter_new( $file, FALSE, undef );
    }
}
...
sub select_directory_new {
    ...
    if ( -e $directory ) {
        ClamTk::Scan->filter_new( $directory, FALSE, undef );
    }
}
...
sub add_default_view {
    ...
    $box->pack_start( add_configuration( _( '自定义' ) ), TRUE, TRUE, 0 );

    $box->pack_start( add_self_panels(), TRUE, TRUE, 0 );
...
sub add_self_panels {
    ...
    my @self_data = (
        {   link        => _( '(新)扫描文件' ),
            description => _( '使用新算法扫描一个文件' ),
            image       => 'filefind',
            button      => FALSE,
        },
        {   link        => _( '（新）扫描目录' ),
            description => _( '使用新算法扫描一个目录' ),
            image       => 'document-open',
            button      => FALSE,
        },
    );

    for my $item ( @self_data ) {
        my $use_image = ClamTk::Icons->get_image($item->{image});
        my $iter = $liststore->append;
        my $pix = Gtk3::IconTheme::get_default->load_icon(
                $use_image, 24, 'use-builtin'
